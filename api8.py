#2nd image overlapping the first image
...
def overlap_images(processed_image_path, overlay_image_path, save_path):
    ...
    # Resize the ROIs from the overlay image to match the size of the ROIs in the processed image
    eyes_roi_overlay = cv2.resize(eyes_roi_overlay, (w, h))

    # Replace the ROIs in the processed image with the corresponding ROIs from the overlay image
    processed_image[y:y + h, x:x + w] = eyes_roi_overlay
