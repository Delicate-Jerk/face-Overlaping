#same as api6 but with ears, not good detection
...
def detect_face_eyes_nose_mouth_ears(image_data):
    ...
    # Load a pre-trained ear detector
    ear_cascade_path = '/Users/arshitarora/Antier-Sol/face-detection/cascade.xml'
    ear_cascade = cv2.CascadeClassifier(ear_cascade_path)
    ...
    for (x, y, w, h) in faces:
        ...
        # Detect ears in the face region
        ears = ear_cascade.detectMultiScale(roi_gray, scaleFactor=1.1, minNeighbors=5, minSize=(30, 30))
        for (ex, ey, ew, eh) in ears:
            cv2.rectangle(img_np, (x + ex, y + ey), (x + ex + ew, y + ey + eh), (0, 255, 255), 2)
...
def upload_image():
    try:
        ...
        if file:
            ...
            processed_image = detect_face_eyes_nose_mouth_ears(image_data)
