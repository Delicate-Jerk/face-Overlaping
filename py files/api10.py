#1st image overlapping 2nd image working fine without rectangles around features
...
def detect_face_eyes_nose_mouth(image_data):
    ...
    for (x, y, w, h) in faces:
        ...
        # Draw circles instead of rectangles for the detected facial features
        # for (ex, ey, ew, eh) in eyes:
        #     cv2.circle(img_np, (x + ex + ew // 2, y + ey + eh // 2), min(ew, eh) // 2, (0, 255, 0), 2)

        # for (nx, ny, nw, nh) in nose:
        #     cv2.circle(img_np, (x + nx + nw // 2, y + ny + nh // 2), min(nw, nh) // 2, (0, 0, 255), 2)

        # for (mx, my, mw, mh) in mouth:
        #     cv2.circle(img_np, (x + mx + mw // 2, y + my + mh // 2), min(mw, mh) // 2, (255, 0, 255), 2)
